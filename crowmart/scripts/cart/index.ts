...
export class CartManager {
  ...
  private renderCartItem(
    ...
    const htmlAsText = `<div>
                            <img class="u-max-full-width" src=${imgSrc}>
                            <h4>${name}</h4><h5>${this.getItemPriceText(price,quantity)}</h5>
  ...
  private getItemPriceText(price: number, quantity: number): string {
    const quantityText = quantity === 1 ? '' : ` x ${quantity}`

    return `$${(price / 100).toFixed(2)}${quantityText}`
  }
  ...
  private async removeItemFromCart(
    ...
    // Insert new state to reflect changes
    const newCartState = {
      items: newItemsState,
      total: newTotal
    }
  ...
  private updateItemsView(): void {
      const cartItemsInDOM = document.getElementById('cart-items')?.childNodes || []
      const cartItemsInState = this.currentCartState.items.reduce((itemMap: { [id: string]: string}, item: CrowMarket.CartItem) => {
        const { id, quantity, price } = item
          itemMap[id] = this.getItemPriceText(price, quantity)
          return itemMap
      }, {})

      cartItemsInDOM.forEach((node) => {
        const buttonNode = Array.from(node.childNodes).find(n => n.nodeName === 'BUTTON') as Element
        const itemId = buttonNode.getAttribute('data-item-id')

        //Remove node if the associated cart item id does not exist in the current state
        if (itemId && !cartItemsInState[itemId]) {
          node.parentNode?.removeChild(node)
        } else {
          const priceInfoNode = Array.from(node.childNodes).find(n => n.nodeName === 'H5') as Element
          priceInfoNode.innerHTML = cartItemsInState[itemId as string]
        }
      })
  }
}
